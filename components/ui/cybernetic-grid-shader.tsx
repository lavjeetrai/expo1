...
const CyberneticGridShader = () => {
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        vec3 gridColor = vec3(0.1, 0.4, 0.8); // Brighter space blue
        ...
        color += vec3(0.0, 1.0, 1.0) * energy * line; // Cyan energy

        // glow around mouse
        float glow = smoothstep(0.2, 0.0, mouseDist);
        color += vec3(0.0, 0.6, 1.0) * glow * 0.8;
  ...
  return (
    <div
      ref={containerRef}
      className="shader-container fixed inset-0 w-full h-full -z-10 pointer-events-none opacity-60"
