...
class Particle {
  ...
  draw() {
    ...
    globalCtx.rotate(this.rotation);
    
    const finalAlpha = Math.min(this.alpha + this.frictionGlow, 0.9);
    globalCtx.fillStyle = `rgba(${this.color}, ${finalAlpha})`;
    
    if (this.frictionGlow > 0.3) {
      globalCtx.shadowBlur = 8 * this.frictionGlow;
      globalCtx.shadowColor = `rgba(180, 220, 255, ${this.frictionGlow})`;
    }

    // Sharp shard geometry
    globalCtx.beginPath();
    globalCtx.moveTo(0, -this.size * 2.5);
    globalCtx.lineTo(this.size, 0);
    globalCtx.lineTo(0, this.size * 2.5);
    globalCtx.lineTo(-this.size, 0);
    globalCtx.closePath();
    globalCtx.fill();
...
export const ASMRStaticBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      globalCtx.fillStyle = 'rgba(10, 10, 12, 0.18)';
  ...
  return (
    <div className="absolute inset-0 w-full h-full overflow-hidden" style={{ zIndex: 0 }}>
      {/* Black bg directly to avoid relying on parent */}
      ...
      {/* Optional UI Overlay (disabled since it will go behind the chat UI anyway, but we keep it minimal) */}
      
      {/* Custom cursor follow */}
      <div 
        className="fixed top-0 left-0 w-4 h-4 rounded-full border border-white/20 pointer-events-none transition-transform duration-75 ease-out z-50 hidden md:block"
        style={{
