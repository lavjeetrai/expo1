...
class Particle {
  ...
  update(mouseX: number, mouseY: number, canInteract: boolean = true) {
    const dx = mouseX - this.x;
    const dy = mouseY - this.y;
    const distance = Math.sqrt(dx * dx + dy * dy);
    const interactionRadius = 120;

    if (canInteract && distance < interactionRadius && mouseX !== -1000 && mouseY !== -1000) {
...
export function CursorDrivenParticleTypography({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const canInteract = requireFastVelocity ? currentVelocity > 3 : true;

      particles.forEach((particle) => {
        particle.update(mouseX, mouseY, canInteract);
        particle.draw(ctx);
      });
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "w-full h-full min-h-[400px] flex items-center justify-center relative touch-none bg-[#0D0D0D] rounded-2xl",
        className
      )}
    >
