...
export const PrismFluxLoader: React.FC<CubeLoaderProps> = ({
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4 h-[220px]">
      ...
      >
        ...
        {statuses.slice(0, 6).map((text, i) => {
          ...
          return (
            <div
              key={i}
              className={`absolute flex items-center justify-center font-semibold text-foreground bg-background`}
              style={{
                width: size,
                height: size,
      ...
      <div
        className="text-sm font-semibold text-foreground tracking-wide mt-8"
      >
        {currentStatus}...
      </div>
    </div>
